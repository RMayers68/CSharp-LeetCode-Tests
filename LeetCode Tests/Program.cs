...
public class Solution
{
    ...
    public static IList<IList<int>> ThreeSum(int[] nums) 
        // Absolutely failed this one by having a O(n^3) time, had to look through solution guides for this one. Sort when applicable!
    {
    ...
    public static int MaxArea(int[] height) 
        // Pretty proud, came up with solution quickly and was 98.8% in speed, GC.Collect makes speed 89% and memory 99%)
    {
        int tmp = 0, lowIndex = 0, highIndex = height.Length-1, maxArea = 0, distance = 0; 
        while(lowIndex < highIndex)
        {
            distance = highIndex - lowIndex;
            if (height[lowIndex] < height[highIndex])
            {
                tmp = height[lowIndex] * distance;
                lowIndex++;
            }
            else
            {
                tmp = height[highIndex] * distance;
                highIndex--;
            }
    ...
    public static string AddBinary(string a, string b)
        // Original solution not good, learned this from leetcode website and improved it (81 ms vs 84ms, 93.1% in memory) 
    {
        var res = "";
        int i = a.Length - 1;
        int j = b.Length - 1;
    ...
    public static int MyAtoi(string s) 
        // 95% speed (50ms) but I feel like it can be compressed. 70% on memory
    {
    ...
    public static int ReverseInteger(int x) 
        // 45% speed, low in memory ( decrease usage by modifying string in place rather than char array)
    {
